defmodule Quintal.VisitasTest do
  ...
  describe "registrar/4" do
    test "dedupa por (tipo, ref_uri): escrita otimista e eco contam uma vez" do
  ...
  describe "resumo/1" do
    test "conta por tipo desde a última passada" do
      ...
      assert Visitas.resumo("did:plc:alice") == %{recado: 2, resposta: 1, novo_leitor: 1, depoimento: 0}
    end
    ...
    test "marcar_lido zera o resumo e apaga a bolinha" do
      ...
      assert Visitas.resumo("did:plc:alice") == %{recado: 0, resposta: 0, novo_leitor: 0, depoimento: 0}
      assert Visitas.novidade?("did:plc:alice") == false

      Visitas.registrar("did:plc:alice", "depoimento", "at://x/2", "did:plc:carol")
      assert Visitas.novidade?("did:plc:alice") == true
      assert Visitas.resumo("did:plc:alice") == %{recado: 0, resposta: 0, novo_leitor: 0, depoimento: 1}
    end
